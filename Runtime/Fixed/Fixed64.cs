...
public static class Fixed64 {
    public const int kFractionalBits = 32;
    public const long kFractionalMask = ( 1L << kFractionalBits ) - 1; // Space before 1L needed because of hacky C++ code generator
    public const long kIntegerMask = ~kFractionalMask;

    // Constants
    public const long Zero = 0L;
    public const long Neg1 = -1L << kFractionalBits;
    public const long One = 1L << kFractionalBits;
    ...
    {
        return (int)((v + (One - 1)) >> kFractionalBits);
    }
    ...
    {
        return (int)((v + Half) >> kFractionalBits);
    }
    ...
    {
        return (x + kFractionalMask) & kIntegerMask;
    }
    ...
    {
        long ai = a >> kFractionalBits;
        long af = (a & kFractionalMask);
        long bi = b >> kFractionalBits;
        long bf = (b & kFractionalMask);
        return FixedUtil.LogicalShiftRight(af * bf, kFractionalBits) + ai * b + af * bi;
    }
    ...
    {
        Debug.Assert(a >= 0);
        int bi = (int)(b >> kFractionalBits);
        long bf = b & kFractionalMask;
        return (int)FixedUtil.LogicalShiftRight(a * bf, kFractionalBits) + a * bi;
    }
    ...
    private static long MulIntLongLong(int a, long b)
    {
        ...
        long bi = b >> kFractionalBits;
        long bf = b & kFractionalMask;
        return FixedUtil.LogicalShiftRight(a * bf, kFractionalBits) + a * bi;
    }
    ...
    public static long DivPrecise(long arg_a, long arg_b)
    {
        ...
        // kFractionalBits amount for norm
        int s = Nlz(v); // 0 <= s <= 63
        ...
        long un10 = u0 << s; // kFractionalBits dividend left
        ...
        ulong un10 = u0 << s; // kFractionalBits dividend left
    ...
    public static long Log2(long x)
    {
        ...
        // Combine integer and fractional parts (into s32.32).
        return ((long)offset << kFractionalBits) + y;
    }
